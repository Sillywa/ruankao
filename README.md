...
- 如果任务保持 `sending` 超过 15 分钟，会先被标记为 `timeout`，随后允许新的自动查询、手动查询或后台重试重新抢锁发送；
...
- 发送成功：一次性授权已消费，`active=false`；
- 授权失效类错误：例如微信返回 `43101`，表示用户未授权、拒收或授权已不可用，`active=false`；
- 临时或未知错误：例如网络波动、系统繁忙、接口临时异常，保留 `active=true`，记录 `failedNoticeUrl` 和错误信息，下次检查同一公告时继续补发。
...
- 每次授权最多发送一条消息；
- 发送成功或授权失效后，代码会将 `active` 更新为 `false`；
